...
Building VibeFinder Agent was my first hands-on experience with agentic AI, and learning LangGraph before implementing the project was one of the best decisions I made. It forced me to think like a system designer rather than just a developer. I had to orchestrate a multi-step workflow, manage shared state across nodes, decide which tools the agent should call and when, write prompts that produced structured and reliable output, handle context across a full conversation, and make sure the system failed gracefully instead of crashing silently. Those are skills that go well beyond just calling an API and displaying a result.

Connecting the agent to a real frontend was another big learning curve. I had some exposure to FastAPI, GraphQL, and Next.js from my internship, but this was the first side project where I deliberately tried to replicate that kind of production tech stack on my own. What it showed me is how much iteration agentic applications actually require. The version I'm submitting is not the most polished or performant thing I've ever built but that's kind of the point. Every bug I fixed, every edge case I handled, and every small improvement I pushed made the system meaningfully better. That compounding improvement is what agentic development actually feels like in practice, and I think it's something you can only really understand by going through it.

Langfuse was something I had heard people talk about for observability, feedback loops, and evaluations, but I had never actually used it. I got to integrate it for observability in this project, and even that alone changed how I debugged and thought about the system. But what I really want to do next is go deeper by wiring Langfuse into a proper evaluation pipeline where another agent analyzes user feedback, scores the recommender's responses, and feeds that signal back to improve future outputs. Right now the system collects feedback but doesn't close that loop autonomously. That's the next level, and this project gave me a clear picture of exactly what it would take to get there.

Overall, this project gave me a real understanding of what it means to build with AI rather than just build AI features. There's a version of this project I could keep improving for months, and I probably will. The ideas don't run out — shipping a working version makes it easier to see what to build next. That's what I find most exciting about agentic development right now.

### Responsible AI Reflection

**Limitations and biases:** The biggest limitation is the catalog because 18 songs is a demo, not a real music library, and it skews toward Western pop, hip-hop, and lofi. The LLM also inherits stereotypes from its training data, so "working out" almost always maps to hip-hop/EDM and "chill" almost always maps to lofi, even when the user might mean something completely different. Confidence scores are LLM-generated and not calibrated against any ground truth, so they cluster between 0.75 and 0.95 regardless of actual match quality.

**Potential misuse:** The catalog is fixed and benign so there is very little misuse risk at this scale. The main concern is users over-trusting the AI's recommendations or confidence scores as authoritative judgments rather than suggestions. The UI addresses this by showing the original rule-based V1 score alongside the AI score on every card, so users can see both signals and decide for themselves.

**What surprised me during testing:** The recursion bug was the most surprising failure because the logs looked completely normal. The bias auditor was reporting "issues found, re-ranking" on every iteration, which is exactly what it's supposed to say, but the `rerank_count` was stuck at 0 because LangGraph doesn't persist state changes made inside conditional edge functions. The system was doing exactly what the code said the problem was that the code was in the wrong place. That kind of subtle runtime boundary issue is not something you can catch by reading the code alone.

**AI collaboration:** The way I worked with AI on this project was intentional from the very beginning. I started in Cursor's Plan Mode where I brought the idea, described the system I wanted to build, and went back and forth with the AI to shape it into a real architectural plan. That planning phase was genuinely collaborative. I would push back on suggestions that didn't fit, ask questions about trade-offs, and the AI would refine the plan based on my constraints. Once we had a plan I felt good about, I moved it to a new chat in Agent Mode to keep context clean and started implementing phase by phase from scaffolding, tools, agent nodes, API layer, frontend, observability, one piece at a time.

During the implementation phases, the dynamic shifted. I would tell the AI what to build next, it would implement it, I would run it, hit an error or see something that didn't work right, paste the output back, and we would iterate. That loop of build, test, observe, and fix repeated dozens of times across the project. The AI was fast at writing the implementation but I was the one running it against a real backend, watching the terminal, and knowing whether the behavior was actually correct or just looked correct in isolation. That combination is what made it work. I learned that AI pair programming is most effective when you stay in the loop at every step rather than delegating whole features and hoping the output is right.

The most helpful suggestion was making the Bias Auditor a self-critique node inside the graph with a conditional back-edge to the Recommender, rather than a post-processing filter outside it. That made re-ranking a native part of the agent's reasoning loop, which is a cleaner and more agentic design. The most flawed suggestion was in the streaming implementation, where the AI wrote `last_state = node_output` assuming each LangGraph node emits a full state snapshot. Nodes emit partial updates, so this caused the backend to save an incomplete session after every turn, silently dropping the user profile on the second message. The fix was `last_state.update(node_output)`, one word but finding it required me to trace through symptoms that looked like a completely different problem. That experience reinforced something I now believe strongly: AI assistance speeds up development significantly, but it doesn't replace the judgment needed to catch subtle system-level bugs.

For the full detailed breakdown see [MODEL_CARD.md](MODEL_CARD.md).

### What This Project Says About Me as an AI Engineer

I care about building things that actually work end to end, not just getting a model to return something reasonable. A lot of AI projects stop at the point where the LLM produces an output. This one goes further, the agent critiques its own recommendations, the system recovers gracefully from server restarts, the frontend streams the agent's reasoning live, and every LLM call is traced and visible to the user. None of those things were required by the assignment. I built them because a system without them isn't one I would be proud to show someone.

The way I used AI throughout this project also says something about how I work. I didn't just prompt and accept. I started in Plan Mode, working back and forth with the AI to turn a rough idea into a real architecture before writing a single line of code. Then I moved to Agent Mode in a fresh chat to keep context clean and implemented phase by phase, running everything myself, pushing back when something didn't behave right, and iterating. That discipline of using AI to move fast but staying in the loop on every decision is what I think effective AI engineering actually looks like. The bugs that mattered most in this project were not caught by the AI. They were caught by me running the system, watching it fail in a specific way, and knowing enough about the internals to understand why.

I also think the way I worked through those bugs says something. The recursion bug, the state accumulation bug, the rate-limit problem in the observability panel none of those were solved by asking the AI to fix them. They were solved by understanding the system well enough to know where the boundary was, what the runtime was actually doing, and why the obvious fix wouldn't work. That kind of thinking is what separates someone who uses AI tools from someone who engineers with them.

This project also showed me that I am genuinely excited about where agentic AI is going. I started with a simple idea and kept seeing more things I wanted to build, a proper eval loop, persistent sessions, Spotify integration, a second agent that analyzes feedback. The list didn't shrink as the project grew, it got longer. That's the kind of problem space I want to keep working in.
