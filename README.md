...
**AI collaboration:** I built this entire project with Cursor as my AI pair programmer — from the initial LangGraph graph structure all the way through debugging SSE streaming and the Langfuse proxy. The way I used it was closer to working with a senior engineer than just generating code. I would describe what I wanted to build, the AI would propose an implementation, and I would review it, test it, and push back when something felt wrong or didn't work as expected. That back-and-forth is where the real learning happened. The AI was fast at scaffolding boilerplate and connecting components, but it consistently needed me to provide the domain context — knowing which LangGraph behavior was causing a bug, knowing that Groq's tool-calling schema is stricter than OpenAI's, knowing that the Langfuse free tier has rate limits that would affect the UI. The collaboration worked well when I came in with a clear question and a specific goal. It worked less well when I let it generate large blocks of code without reviewing them carefully first, which is exactly how the `last_state` bug got introduced.
